...
class CreateCollectionRequest(BaseModel):
    ...
    distance: str = Field(
        ..., description="Distance metric to use", pattern="^(cosine|dot)$"
    )
...
class InsertPointRequest(BaseModel):
    ...
    collection_name: str = Field(
        ..., description="Name of the collection to insert the point into"
    )
...
class SearchPointsRequest(BaseModel):
    ...
    limit: Optional[int] = Field(
        default=10, description="Maximum number of points to return", gt=0, le=100
    )
...
class DeleteDocumentRequest(BaseModel):
    ...
    collection_name: str = Field(
        ..., description="Name of the collection to delete the document from"
    )
